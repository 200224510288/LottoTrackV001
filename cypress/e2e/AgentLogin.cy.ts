describe('Clerk Login Page', () => {
    ...
    it('should render login form with Clerk fields', () => {
      ...
      it('should allow successful agent login and redirect', () => {
        cy.get('input[type="text"]').first().type('Aloka1234');
        cy.get('input[type="password"]').type('Aloka@0777');
    
        cy.get('button').contains('Login').click();
    
        // After login, should redirect based on role (e.g., /agent or /user)
        cy.url().should('match', /\/(agent|user|dashboard)/); 
      });
    });